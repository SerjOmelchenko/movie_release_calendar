...
const HEADER_SVG = `<svg class="brand-icon" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <circle cx="22" cy="22" r="20"   stroke="#2a2a2a" stroke-width="1.2"/>
      <circle cx="22" cy="22" r="13.5" stroke="#2a2a2a" stroke-width="1.2"/>
      <circle cx="22" cy="22" r="7"    stroke="#2a2a2a" stroke-width="1.2"/>
      <line x1="22" y1="2"  x2="22" y2="42" stroke="#1e1e1e" stroke-width="1"/>
...
const GTM_SNIPPET = `<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-T3BJFZSV');<\/script>`;
...
function buildTopMoviesPage(ym, topMovies) {
  ...
  const ogImage = topMovies[0]?.poster_path
    ? escHtml(`${IMG_BASE}w500${topMovies[0].poster_path}`)
    : '';
  ...
  const moviesHtml = topMovies.map((m, i) => {
    ...
    const relDate    = m.release_date
      ...
      : 'TBA';
    const ratingHtml = m.vote_count > 0
    ...
    <article class="top-item">
      ...
      <div class="poster-wrap">
        ${poster
          ? `<a href="/movie/${m.slug}/"><img src="${poster}" alt="${title} poster" loading="lazy" width="150" height="225" /></a>`
          : `<div class="poster-placeholder"><span>${title}</span></div>`}
      </div>
      <div class="movie-info">
        <h2><a href="/movie/${m.slug}/">${title}</a></h2>
        <div class="meta-row">
...
<head>
  ...
  <style>${SHARED_STYLES}
    ...
    .rank { position: absolute; top: -10px; left: -10px; width: 32px; height: 32px; background: #e94560; color: #fff; font-weight: 800; font-size: 0.9rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(233,69,96,0.5); flex-shrink: 0; }
...
function buildTopMoviesIndexPage(allMonths, detailedMovies) {
  ...
  const metaDesc     = escHtml(
    'Browse the top 10 most anticipated movies for each month, ranked by popularity. Updated daily.'
  );
  const today = new Date().toISOString().slice(0, 10);

  // Pre-index movies by month for quick lookup
  const moviesByMonth = {};
  ...
  const monthCardsHtml = allMonths.map(ym => {
    ...
    const thumbsHtml = topThree.map(m => {
      const src = m.poster_path ? `${IMG_BASE}w154${m.poster_path}` : '';
      return src
        ? `<img src="${escHtml(src)}" alt="${escHtml(m.title)} poster" loading="lazy" width="60" height="90" />`
        : '';
    }).join('');
...
<head>
  ...
  <style>${SHARED_STYLES}
    ...
    .month-thumbs img { width: 54px; height: 81px; object-fit: cover; border-radius: 6px; flex-shrink: 0; }
...
function generateTopMoviesPages(detailedMovies) {
  ...
  // Always include one month ahead so next month's page exists at start of month
  const nextYm = addMonths(currentYm, 1);

  // Collect all months from TOP_MOVIES_START through nextYm
  const allMonths = [];
  ...
  for (const ym of allMonths) {
    const monthDir = path.join(TOP_MOVIES_DIR, ym);
    const outPath  = path.join(monthDir, 'index.html');

    // Past months: generate once, then freeze
    if (ym < currentYm && fs.existsSync(outPath)) {
      skipped++;
      continue;
    }
  ...
  // Always regenerate the index page (it lists all months)
  fs.writeFileSync(
    path.join(TOP_MOVIES_DIR, 'index.html'),
    buildTopMoviesIndexPage(allMonths, detailedMovies)
  );
